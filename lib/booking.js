...
import { partners } from "./firm-data";
...
export function partnerEmail(partnerId) {
  const partner = getPartner(partnerId);
  return process.env[partner.emailEnv] || process.env.SUZANA_EMAIL || "admin.kl@suzanaali.com";
}
...
export async function availabilityFor(partnerId) {
  const booked = new Set(
    (await readBookings())
      .filter((booking) => booking.partnerId === partnerId)
  ...
  for (let offset = 1; slots.length < 12 && offset < 21; offset += 1) {
    ...
    for (const time of officeTimes) {
      const slotId = `${partnerId}:${dateKey}:${time}`;
